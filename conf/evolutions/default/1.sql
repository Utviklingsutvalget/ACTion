...
alter table board_membership add constraint fk_board_membership_boardPost_2 foreign key (board_post_id) references board_post (id) on delete restrict on update restrict;
create index ix_board_membership_boardPost_2 on board_membership (board_post_id);
alter table board_membership add constraint fk_board_membership_user_3 foreign key (user_id) references user (id) on delete restrict on update restrict;
create index ix_board_membership_user_3 on board_membership (user_id);
alter table club add constraint fk_club_location_4 foreign key (location_id) references location (id) on delete restrict on update restrict;
create index ix_club_location_4 on club (location_id);
